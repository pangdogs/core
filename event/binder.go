...
// BindEvent 绑定事件与订阅者，可以设置优先级调整回调先后顺序，按优先级升序排列
func BindEvent[T any](event IEvent, delegate T, priority ...int32) Hook {
	if event == nil {
	...
	return event.newHook(iface.MakeFacePair[any](delegate, delegate), _priority)
}
