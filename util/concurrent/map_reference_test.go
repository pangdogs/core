...
func (m *ConcurrentMap[K, V]) Load(key any) (value any, ok bool) {
	return m.cmap.Load(key.(K))
}

func (m *ConcurrentMap[K, V]) Store(key, value any) {
	m.cmap.Store(key.(K), value.(V))
}

func (m *ConcurrentMap[K, V]) LoadOrStore(key, value any) (actual any, loaded bool) {
	return m.cmap.LoadOrStore(key.(K), value.(V))
}

func (m *ConcurrentMap[K, V]) LoadAndDelete(key any) (value any, loaded bool) {
	return m.cmap.LoadAndDelete(key.(K))
}

func (m *ConcurrentMap[K, V]) Delete(key any) {
	m.cmap.Delete(key.(K))
}

func (m *ConcurrentMap[K, V]) Range(f func(key, value any) (shouldContinue bool)) {
	m.cmap.Range(func(k K, v V) bool {
		return f(k, v)
	})
}
