...
Golaxy Core 是 [Golaxy 分布式服务开发框架](https://github.com/pangdogs/framework) 的执行内核和编程模型基础。它以 Actor 风格的 Runtime 串行执行域承载 EC（Entity-Component）业务对象，并提供生命周期、原型、实体树、进程内事件、add-in 和 Future/Await 等基础能力。
...
- **异步协作**：提供 Runtime 调度、后台 goroutine、定时器、Channel 转 Future，以及 `Any`、`OK`、`All`、`Transform`、`Foreach` 等 Await 策略。
- **同步事件**：提供带优先级、递归策略、托管解绑和代码生成的进程内 signal/slot 事件系统。
- **Add-in 扩展**：区分固定启动集合的 Service add-in 与可热插拔的 Runtime add-in。
- **生命周期与统计**：统一驱动 Service、Runtime、Entity、Component 和 Add-in，并暴露等待组、任务队列与帧统计。
...
flowchart TB
    ...
    subgraph RuntimeScope[Runtime 串行执行域]
        RuntimeObject[core.Runtime] --> RuntimeContext[runtime.Context]
        ExternalWork[外部 goroutine / 定时器 / I/O] -->|CallAsync / Await| TaskQueue[任务队列]
...
| `runtime.ConcurrentContext` | 暴露可跨 goroutine 使用的 Runtime 子集。 | 跨 goroutine 通过 `CallAsync` 回到运行线程。 |
...
sequenceDiagram
    ...
    Source->>Queue: CallAsync 或 Await 回调入队
...
- Runtime 外部不能直接修改 Entity、Component、EntityTree 或 Runtime add-in；应使用 `CallAsync`、`CallVoidAsync` 或按实体 ID 调用。
- `GoAsync` 适合阻塞 I/O 和独立计算，但后台函数不能直接触碰 Runtime 局部状态；使用 `Await` 把结果处理重新投递回来。
...
- `service.Context.CallAsync(entityId, ...)` 先从全局索引取得 `ConcurrentEntity`，再把调用投递到目标 Runtime。
...
| `Terminated` | Service add-in 已按逆序关闭，终止 Future 完成。 |
...
| `Terminated` | 等待组清空、Runtime add-in 停用，托管事件句柄解绑，终止 Future 完成。 |
...
6. Entity 销毁时会取消其 Context、移出实体树与索引，并自动解绑 Entity/Component 托管的事件句柄。

## Runtime 调度与帧循环

### 任务队列

普通调用由 `runtime.Context.CallAsync`、`CallVoidAsync` 或根包同名辅助函数进入任务队列。

- 默认使用**无界队列**；`Capacity=128` 仅在切换为有界队列后生效。
- 有界队列使用非阻塞入队；容量满时 Future 返回 `ErrTaskQueueFull`。
- Runtime 关闭队列后，新任务返回 `ErrTaskQueueClosed`。
- 终止时会排空已经接收的任务，然后执行最终 GC。
- `Runtime.Stats()` 按普通调用和帧任务分别统计 `Enqueued`、`Pending`、`Rejected` 与 `Completed`。

无界队列避免生产者因瞬时高峰立即失败，但积压会消耗内存并提高延迟。生产环境应对 `Pending`、调用耗时和外部入口做限流与监控。
...
`utils/async` 提供 `Result`、`Future`、`FutureChan` 和 `FutureVoid`：

- `Future.Wait(ctx)` 等待下一项结果或 Context 取消。
- `Future.Chan()` 消费一次或多次 yield 结果。
- `Future.Done()` 在生产者结束 Future 时关闭。
- `Future.Context(ctx)` 创建随 Future 完成而取消的派生 Context。
- Future 的结果频道是**竞争消费**而不是广播；多个消费者会争抢同一批结果。
- 生产者必须选择一次 `Return`，或者多次 `YieldReturn` 后执行一次 `YieldBreak`。

### 异步入口

| API | 执行位置 | 用途 |
| --- | --- | --- |
| `CallAsync` / `CallVoidAsync` | 目标 Runtime goroutine | 安全读取或修改 Runtime 局部状态。 |
| `GoAsync` / `GoVoidAsync` | 新 goroutine | 阻塞 I/O、独立计算或不触碰 Runtime 状态的工作。 |
| `TimeAfterAsync` / `TimeAtAsync` | 内部定时 goroutine | 一次性定时结果。 |
| `TimeTickAsync` | 内部定时 goroutine | 持续产生 tick，直到 Context 取消。 |
| `ReadChanAsync` | 内部桥接 goroutine | 把 Channel 的连续值转换为 Future 产出。 |

### Await

`Await(provider, futures...)` 在后台等待 Future，但把后续回调重新调度到 `provider` 所属 Runtime：

| 策略 | 语义 |
| --- | --- |
| `Any` / `AnyVoid` | 使用最先可用的一项结果，不区分成功或失败。 |
| `OK` / `OKVoid` | 使用第一个成功结果；全部失败时返回 `ErrNoFutureSucceeded`。 |
| `All` / `AllVoid` | 从每个 Future 取得一项结果，并按传入顺序提供。 |
| `Transform` | 合并连续产出，在 Runtime 中逐项转换并继续产出。 |
| `Foreach` | 合并连续产出，在 Runtime 中逐项执行无返回值回调。 |

不要在 Runtime goroutine 中直接长时间调用 `Future.Wait`；这会阻塞整个串行域。通常应使用 `Await` 形成“后台等待 → 回到 Runtime 更新状态”的流程。
...
- Entity Context 默认派生自所属 Runtime Context。
...
| [`/ec`](./ec) | Entity/Component 模型、状态机、组件管理、作用域和树节点事件。 |
...
| [`/utils/async`](./utils/async) | Future、Result、Return 与 Yield 基础设施。 |
| [`/utils/corectx`](./utils/corectx) | Service/Runtime 共用 Context、等待组和关闭协议。 |
