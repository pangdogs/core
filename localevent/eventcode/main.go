...
//     `go:generate go run github.com/galaxy-kit/galaxy-go/localevent/eventcode --decl_file=$GOFILE gen_eventtab --package=$GOPACKAGE --name=XXXEventTab`
//
...
func main() {
	...
	eventPackageAlias := kingpin.Flag("event_package_alias", "导入Galaxy框架的`github.com/galaxy-kit/galaxy-go/localevent`包时使用的别名。").Default("localevent").String()
